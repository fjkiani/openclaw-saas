  ...
  try {
    ...
    for (const s of ZOA_SKILLS) {
      const res = await client.query(`
        ...
        ON CONFLICT (slug) DO UPDATE SET name=EXCLUDED.name, description=EXCLUDED.description, category=EXCLUDED.category, featured=EXCLUDED.featured
        RETURNING id
      `, [s.name, s.slug, s.description, s.category, s.featured, ["zoa","multi-agent",s.category.toLowerCase()]]);
      skillIds[s.slug] = res.rows[0].id;
      console.log(`  skill: ${s.name} id=${res.rows[0].id}`);
    }
    ...
    for (const s of ZOA_SKILLS) {
      ...
      `, [skillIds[s.slug], `demo-${s.slug}`, s.l4, JSON.stringify({l1:s.l1,l2:s.l2,l3:s.l3,l4:s.l4}), s.grade, JSON.stringify({grade:s.grade,levelScores:{l1:s.l1,l2:s.l2,l3:s.l3,l4:s.l4}})]);
      console.log(`  benchmark: ${s.slug} → ${s.grade}`);
    }

    const tenantId = "tenant-demo-openclaw";
    await client.query(`
      INSERT INTO tenants (id, name, user_id, plan) VALUES ($1,'Demo Workspace',$2,'free')
      ON CONFLICT (id) DO UPDATE SET user_id=EXCLUDED.user_id, name=EXCLUDED.name
    `, [tenantId, DEMO_USER_ID]);
    console.log(`  tenant: ${tenantId} → ${DEMO_USER_ID}`);

    for (const s of ZOA_SKILLS) {
      await client.query(`
        INSERT INTO tenant_skills (tenant_id, skill_id, enabled) VALUES ($1,$2,true)
        ON CONFLICT DO NOTHING
      `, [tenantId, skillIds[s.slug]]);
    }
    console.log("  installed 6 skills on tenant");
    ...
    console.log(`\n✅ Done: ${count} total skills, ${bcount} benchmarks`);
  } finally {
    client.release();
    await pool.end();
  }
