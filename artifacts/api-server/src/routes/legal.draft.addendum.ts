...
const VENDOR_POLICY_VERSION = "v1.0.0";
...
// ── ReviewThreshold ordering (local helper) ──────────────────────────────────
const THRESHOLD_ORDER_LOCAL = [
  "self_review_ok",
...
function maxThresholdLocal(
  a: import("../lib/draftReceiptEngine").ReviewThreshold,
  b: import("../lib/draftReceiptEngine").ReviewThreshold,
): import("../lib/draftReceiptEngine").ReviewThreshold {
  return THRESHOLD_ORDER_LOCAL.indexOf(a as any) >= THRESHOLD_ORDER_LOCAL.indexOf(b as any) ? a : b;
}
...
router.post("/v1/legal/draft/analyze", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ── 1 & 2. Extract (Lane A) + Coverage review (Lane B) — parallel ───────
    const [extractResult, coverageResult] = await Promise.all([
      extractIntakeFromText(contract_text, validDocClass),
      Promise.resolve(reviewDocumentCoverage(contract_text, validDocClass)),
    ]);
    ...
    // ── 4. Build draft from normalized + safe-defaulted intake ──────────────
    ...
    // ── 6. Governance — max of Lane A + Lane B thresholds ───────────────────
    const governanceResult = buildDraftGovernance(verifierResult);
    const finalThreshold = maxThresholdLocal(
      governanceResult.review_threshold,
    ...
    return res.status(200).json({
      ...
      trace: {
        ...
        vendor_policy_version: VENDOR_POLICY_VERSION,
      },
    });
  } catch (err) {
