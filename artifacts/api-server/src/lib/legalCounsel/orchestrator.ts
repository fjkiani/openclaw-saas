...
import type { LensInput, OrchestratorResult } from "./types.js";
...
export async function runOrchestrator(input: OrchestratorInput): Promise<OrchestratorResult> {
  ...
  // ── Run 4 lens agents in parallel ────────────────────────────────────────
  logger.info({ perspective }, "orchestrator: starting 4 parallel lens agents");

  const [taxResult, delawareResult, ipResult, regResult] = await Promise.all([
    runTaxSecuritiesLens(lensInput),
    runDelawareCorpLens(lensInput),
    runIpAssignmentLens(lensInput),
    runRegulatoryEmploymentLens(lensInput),
  ]);
