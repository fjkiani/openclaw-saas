...
// Structural Pool type — avoids a hard dependency on @types/pg.
// The live benchmark harness does not use a database pool at all; this type
// exists only so callers that pass one (for eval_run persistence) type-check.
...
async function jsonRpcCall(
  ...
  id: number | null,
  sessionId?: string,
): Promise<{ ok: boolean; latency_ms: number; body: unknown; status?: number; error?: string; sessionId?: string }> {
  ...
  try {
    ...
    // id=null signals a JSON-RPC notification (no response expected).
    const body = id === null ? JSON.stringify({ jsonrpc: "2.0", method, params }) : JSON.stringify({ jsonrpc: "2.0", id, method, params });
    ...
    // Capture session ID from the initialize response (session-based servers).
    const respSessionId = res.headers.get("MCP-Session-Id") ?? undefined;
    let parsed: unknown = null;
    try {
      const text = await res.text();
      // SSE: scan every line for "data:" (some servers prepend "event: message").
      const dataLine = text.split("\n").find((l) => l.startsWith("data: "));
      if (dataLine) {
        parsed = JSON.parse(dataLine.slice(6));
      } else if (text.startsWith("data:")) {
        parsed = JSON.parse(text.slice(5).trim());
      } else {
    ...
    return { ok: res.ok, latency_ms, body: parsed, status: res.status, sessionId: respSessionId };
  } catch (err) {
...
async function liveBenchmark(mcpSlug: string, mcpUrl: string, declaredTools: string[]): Promise<BenchmarkResult> {
  ...
  // Session-based servers (e.g. context7) issue an MCP-Session-Id header on
  // initialize and require it on every subsequent request. Capture it and
  // relay it; also send the required notifications/initialized notification
  // for session-based servers before any further calls.
  const sessionId = init.sessionId;
  if (sessionId) {
    await jsonRpcCall(mcpUrl, "notifications/initialized", {}, null, sessionId);
  }
  tasks.push({
    task: "initialize",
    category: "handshake",
  ...
  const list = await jsonRpcCall(mcpUrl, "tools/list", {}, 2, sessionId);
  ...
  for (const toolName of probeTools) {
    ...
    const probe = await jsonRpcCall(mcpUrl, "tools/call", { name: toolName, arguments: {} }, idCounter++, sessionId);
    tasks.push({
      task: toolName,
      category: "tool_call",
...
export async function benchmarkMcp(
  ...
  // DB persistence is optional — skip when no pool is provided (e.g. the rigor
  // route runs the benchmark for its result without persisting to evaluation_runs).
  if (pool) {
  ...
  try {
    ...
    result.eval_run_id = runRes.rows[0].id as number;
